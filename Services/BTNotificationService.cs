	...
	public class BTNotificationService : IBTNotificationService
	{
		...
		public async Task NotificationsByRoleAsync(int? companyId, Notification? notification, BTRoles role)
		{
			try
			{
				if (notification != null)
				{
					IEnumerable<string> memberIds = (await _rolesService.GetUsersInRoleAsync(role.ToString(), companyId))!.Select(u => u.Id);
		...
		public async Task<bool> NewDeveloperNotificationAsync(int? ticketId, string? developerId, string? senderId)
		{
			try
			{
				...
				Notification? notification = new()
				{
					...
					NotificationTypeId = _context.NotificationTypes.FirstOrDefault(nt => nt.Name == nameof(BTNotificationType.Ticket))!.Id
				};
		...
		public async Task<bool> NewTicketNotificationAsync(int? ticketId, string? senderId)
		{
			...
			if (ticket != null && btUser != null)
			{
				Notification? notification = new()
				{
					...
					NotificationTypeId = _context.NotificationTypes.FirstOrDefault(nt => nt.Name == BTNotificationType.Ticket.ToString())!.Id
				};
		...
		public async Task<bool> SendEmailNotificationByRoleAsync(int? companyId, Notification? notification, BTRoles role)
		{
			try
			{
				...
				if (notification != null)
				{
					IEnumerable<string> memberEmails = (await _rolesService.GetUsersInRoleAsync(role.ToString(), companyId))!.Select(u => u.Email)!;
